...
/*   Updated: 2017/08/08 13:34:58 by fflorens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	copy_sym(t_nlist64 sym, void *str, t_file *f, uint32_t *idx)
{
...
static int	crawl_section(t_file *f, uint32_t ncmds, uint32_t sect_nb)
{
	...
	while (i[0] < ncmds)
	{
		...
		{
			i[1] = 0;
			sect = (void *)lc + sizeof(t_segc64);
			while (i[1]++ < ((t_segc64 *)lc)->nsects)
			{
				if ((void *)sect + sizeof(t_sect64) > f->ptr + f->size)
					return (ft_nm_err(f->name, STR_LC_TOO_MANY, 0));
				sect = copy_sect64(sect, f, i[2]++);
			}
		}
		i[0]++;
...
static int	crawl_sym(t_file *f, t_symc *sym, char swap)
{
	...
	off = 0;
	i = 0;
	swap_nlist64(st, sym->nsyms, swap);
	while (i < sym->nsyms)
	{
...
static int	handle_seg64(t_lc *lc, uint32_t *nb, t_file *file, char swap)
{
	t_segc64	*seg;
	t_symc		*sym;
...
int			nm_handle_64(t_file *f, char swap)
{
	t_mhdr64		*hdr;
	t_lc			*lc;
	uint32_t		sect_nb;
	...
	if (f->ptr + f->size < f->ptr + sizeof(t_mhdr64))
		return (ft_nm_err(f->name, STR_FILE_TOO_SMALL, 0));
	setflags64(f, f->ptr);
	hdr = f->ptr;
	...
	while (i++ < hdr->ncmds)
	{
		...
		if (test_lc_64(lc, f) != EXIT_SUCCESS
				|| handle_seg64(lc, &sect_nb, f, swap) != EXIT_SUCCESS)
			return (EXIT_FAILURE);
		lc = (void *)lc + lc->cmdsize;
	}
