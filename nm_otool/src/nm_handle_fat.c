...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/05/24 13:17:55 by fflorens          #+#    #+#             */
/*   Updated: 2017/08/09 11:46:21 by fflorens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	process_arch(t_file *file, t_farch *arch, int has_arch)
{
	...
	f.archname[0] = 0;
	set_arch(&f, arch);
	f.flag = (has_arch ? f.flag : (f.flag | FLAG_ARCH));
...
int			nm_handle_fat(t_file *f, char swap)
{
	t_fhdr	*hdr;
	t_farch	*arch;
	int		res;
	...
	if ((void *)arch + sizeof(t_farch) > f->ptr + f->size ||
		...
		return (ft_nm_err(f->name, "' file size too small\n", 0));
	...
	if (f->best_arch)
		...
		return (ft_nm_err(f->name, g_arch, 1));
	while (hdr->nfat_arch--)
		if ((res = process_arch(f, arch++, 1)) == EXIT_FAILURE)
			return (EXIT_FAILURE);
	return (EXIT_SUCCESS);
}
