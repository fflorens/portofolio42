...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/08/08 13:38:00 by fflorens          #+#    #+#             */
/*   Updated: 2017/08/08 15:57:40 by fflorens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check_seg(t_segc *seg, t_sect *currsect, t_file *file)
{
	if (seg->nsects > UINT_MAX / sizeof(t_sect) ||
		sizeof(t_sect) * seg->nsects > ((t_lc *)seg)->cmdsize - sizeof(*seg))
...
static int		crawl_command(t_segc *seg, t_file *file, char swap)
{
	t_sect		*currsect;
	uint32_t	i;
	...
	if (check_seg(seg, currsect, file) != EXIT_SUCCESS)
		return (EXIT_FAILURE);
	swap_section(currsect, seg->nsects, swap);
	while (i++ < seg->nsects)
	{
		if ((void *)currsect + sizeof(t_sect) > file->ptr + file->size)
			return (ft_otool_err(file->name, STR_FILE_TOO_SMALL, 0));
		if (!ft_strcmp(currsect->sectname, SECT_TEXT) &&
			!ft_strcmp(currsect->segname, SEG_TEXT))
		{
			file->sec = copy_sec(currsect, file);
...
static int		test_sym(t_file *f, t_symc *sym, char swap)
{
	...
	i = 0;
	j = 0;
	swap_nlist(symt, sym->nsyms, swap);
	while (i < sym->nsyms)
	{
...
int				otool_handle_32(t_file *f, char swap)
{
	t_mhdr		*hdr;
	t_lc		*lc;
	uint32_t	i;

	if ((off_t)f->size < (off_t)sizeof(t_mhdr))
		return (ft_otool_err(f->name, STR_FILE_TOO_SMALL, 0));
	hdr = f->ptr;
